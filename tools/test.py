        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if args.vis and idx > frame_counter:
                    cv2.polylines(img, [np.array(gt_bbox, np.int).reshape(
                        (-1, 1, 2))], True, (0, 255, 0), 3)
                    if cfg.MASK.MASK:
                        cv2.polylines(img, [np.array(
                            pred_bbox, np.int).reshape((-1, 1, 2))],
                            True, (0, 255, 255), 3)
                    else:
                ...
                for x in pred_bboxes:
                    if isinstance(x, int):
                        ...
                        f.write(','.join([vot_float2str("%.4f", i)
                                          for i in x]) + '\n')
            print('({:3d}) Video: {:12s} Time: {:4.1f}s Speed: {:3.1f}\
                fps Lost: {:d}'.format(v_idx + 1, video.name, toc,
                                       idx / toc, lost_number))
        ...
        for v_idx, video in enumerate(dataset):
            ...
            if 'VOT2018-LT' == args.dataset:
                ...
                video_path = os.path.join(
                    'results', args.dataset, model_name, video.name)
                if not os.path.isdir(video_path):
                    os.makedirs(video_path)
                result_path = os.path.join(
                    video_path, '{}_001.txt'.format(video.name))
