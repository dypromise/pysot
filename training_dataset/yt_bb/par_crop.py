...
def dl_and_cut(vid, data, d_set_dir):
    for index, row in data.iterrows():
        ...
        z, x = crop_like_SiamFC(
            image, bbox, instanc_size=instanc_size, padding=avg_chans)
        cv2.imwrite(join(crop_class_dir, '{:06d}.{:02d}.z.jpg'.format(
            int(timestamp_ms) / 1000, int(object_id))), z)
        cv2.imwrite(join(crop_class_dir, '{:06d}.{:02d}.x.jpg'.format(
            int(timestamp_ms) / 1000, int(object_id))), x)
    return True
...
def parse_and_sched(dl_dir='.', num_threads=24):
    ...
    for d_set in d_sets:
        ...
        df = pd.DataFrame.from_csv(
            d_set + '.csv', header=None, index_col=False)
        ...
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            fs = [executor.submit(
                dl_and_cut, vid, df[df['youtube_id'] == vid], d_set_dir) for vid in vids]
