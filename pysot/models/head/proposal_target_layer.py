...
class _ProposalTargetLayer(nn.Module):
    ...
    def _compute_targets_pytorch(self, ex_rois, gt_rois):
        ...
        if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
            ...
            targets = ((targets - self.BBOX_NORMALIZE_MEANS.expand_as(
                targets)) / self.BBOX_NORMALIZE_STDS.expand_as(targets))
    ...
    def _sample_rois_pytorch(self, all_rois, gt_boxes, fg_rois_per_image,
        ...
        examples.
        input:
            all_rois: (b, num_proposals, 5)
            gt_boxes: (b, num_classes(1), 5)

        return:
            labels_batch: (b, rois_per_image)
            rois_batch: (b, rois_per_image, 5)
            bbox_targets: (b, rois_per_image, 4)
            bbox_inside_weights: (b, rois_per_image, 4)
        """
        # overlaps: (rois x gt_boxes)
        overlaps = bbox_overlaps_batch(all_rois, gt_boxes)

        # find max overlap gt_box for each roi
        ...
        for i in range(batch_size):
            ...
            if fg_num_rois > 0 and bg_num_rois > 0:
                ...
                # torch.randperm seems has a bug on multi-gpu setting that
                # cause the segfault.
                # See https://github.com/pytorch/pytorch/issues/1868 for
                # more details. use numpy instead.
                ...
                # Seems torch.rand has a bug, it will generate very large
                # number and make an error. We use numpy rand instead.
                ...
                raise ValueError("bg_num_rois = 0 and fg_num_rois = 0,"
