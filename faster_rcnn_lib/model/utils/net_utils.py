...
def _crop_pool_layer(bottom, rois, max_pool=True, stride=16.0):
    # code modified from
    # https://github.com/ruotianluo/pytorch-faster-rcnn
    # implement it using stn box to affine
    # input (x1,y1,x2,y2)
    """
    [  x2-x1             x1 + x2 - W + 1  ]
    [  -----      0      ---------------  ]
    ...
    x1 = rois[:, 1::4] / stride  # keep dims op
    y1 = rois[:, 2::4] / stride
    x2 = rois[:, 3::4] / stride
    y2 = rois[:, 4::4] / stride
    ...
    theta = torch.cat([
        (x2 - x1) / (width - 1), zero, (x1 + x2 - width + 1) / (width - 1),
        zero, (y2 - y1) / (height - 1), (y1 + y2 - height + 1) / (height - 1)],
        1).view(-1, 2, 3)
    ...
    if max_pool:
        ...
        bottom = bottom.view(batch_size, 1, D, H, W).contiguous().expand(
            batch_size, roi_per_batch, D, H, W).contiguous().view(-1, D, H, W)
...
def _affine_grid_gen(rois, input_size, grid_size, stride=16.0):
    rois = rois.detach()
    x1 = rois[:, 1::4] / stride
    y1 = rois[:, 2::4] / stride
    x2 = rois[:, 3::4] / stride
