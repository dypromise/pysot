...
def bbox_transform_batch(ex_rois, gt_rois):
    ...
    if ex_rois.dim() == 2:
        ...
        targets_dx = (gt_ctr_x - ex_ctr_x.view(
            1, - 1).expand_as(gt_ctr_x)) / ex_widths
        targets_dy = (gt_ctr_y - ex_ctr_y.view(
            1, - 1).expand_as(gt_ctr_y)) / ex_heights
...
def bbox_transform_inv2(boxes_center, deltas, batch_size):
    ctr_x = boxes_center[:, :, 0]
    ctr_y = boxes_center[:, :, 1]
    widths = boxes_center[:, :, 2]
    heights = boxes_center[:, :, 3]
